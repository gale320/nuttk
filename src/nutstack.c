 ...
 *
 * @return NUT_OK if the creation was successful, or CC_ERR_ALLOC if the
 * memory allocation for the new Stack structure failed.
 */
NutState nut_stack_new(Stack **out)
{
    StackConf conf;
    nut_stack_conf_init(&conf);
    return nut_stack_new_conf(&conf, out);
}
 ...
 * @param[out] out pointer to where the newly created Stack is to be stored
 *
 * @return NUT_OK if the creation was successful, CC_ERR_INVALID_CAPACITY if
...
NutState nut_stack_new_conf(StackConf const * const conf, Stack **out)
{
    ...
    Array *array;
    NutState status;
    if ((status = nut_array_new_conf(conf, &array)) == NUT_OK) {
        stack->v = array;
    } else {
        conf->mem_free(stack);
        return status;
...
{
    nut_array_destroy_cb(stack->v, cb);
    free(stack);
}
 ...
 *
 * @return NUT_OK if the element was successfully pushed, or CC_ERR_ALLOC
 * if the memory allocation for the new element failed.
 */
NutState nut_stack_push(Stack *stack, void *element)
{
    return nut_array_add(stack->v, element);
}
 ...
 *
 * @return NUT_OK if the element was found, or CC_ERR_VALUE_NOT_FOUND if the
 * Stack is empty.
 */
NutState nut_stack_peek(Stack *stack, void **out)
{
    return nut_array_get_last(stack->v, out);
}
 ...
 *                 to be ignored
 *
 * @return NUT_OK if the element was successfully popped, or CC_ERR_OUT_OF_RANGE
 * if the Stack is already empty.
 */
NutState nut_stack_pop(Stack *stack, void **out)
{
    return nut_array_remove_last(stack->v, out);
}
...
{
    nut_array_iter_init(&(iter->i), s->v);
}
...
{
    return nut_array_iter_next(&(iter->i), out);
}

/**
 * Replaces the last returned element by <code>nut_stack_iter_next()</code>
 * with the specified element and optionally sets the out parameter to
 * the value of the replaced element.
 *
 * @note This function should only ever be called after a call to <code>
 * nut_stack_iter_next()</code>.
 *
...
{
    return nut_array_iter_replace(&(iter->i), element, out);
}
 ...
 */
void nut_stack_zip_iter_init(StackZipIter *iter, Stack *s1, Stack *s2)
{
    nut_array_zip_iter_init(&(iter->i), s1->v, s2->v);
}
 ...
 *
 * @return NUT_OK if a next element pair is returned, or CC_ITER_END if the end
 * of one of the stacks has been reached.
 */
NutState nut_stack_zip_iter_next(StackZipIter *iter, void **out1, void **out2)
{
    return nut_array_zip_iter_next(&(iter->i), out1, out2);
}

/**
 * Replaces the last returned element pair by <code>nut_stack_zip_iter_next()</code>
 ...
 *
 * @return NUT_OK if the element was successfully replaced, or CC_ERR_OUT_OF_RANGE.
 */
NutState nut_stack_zip_iter_replace(StackZipIter *iter, void *e1, void *e2, void **out1, void **out2)
{
    return nut_array_zip_iter_replace(&(iter->i), e1, e2, out1, out2);
}
