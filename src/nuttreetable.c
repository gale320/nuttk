...
NutState nut_treetable_new(int (*cmp) (const void*, const void*), TreeTable **tt)
{
    ...
    conf.cmp = cmp;
    return nut_treetable_new_conf(&conf, tt);
}
 ...
 * @param[out] out  Pointer to where the returned value is stored
 *
 * @return NUT_OK if the key was found, or NUT_ERR_VALUE_NOT_FOUND if not.
 */
 ...
 *
 * @return NUT_OK if the operation was successful, or NUT_ERR_ALLOC if the memory
 ...
 */
void nut_treetable_foreach_key(TreeTable *table, void (*fn) (const void *k))
{
 ...
 *               table
 */
void nut_treetable_foreach_value(TreeTable *table, void (*fn) (void *k))
{
    RBNode *n = tree_min(table, table->root);
...
/**
 * Removes the last returned entry by <code>nut_treetable_iter_next()</code>
 ...
 * NUT_ERR_KEY_NOT_FOUND if the entry was already removed.
 */
...
#ifdef DEBUG
static int nut_treetable_test(TreeTable *table, RBNode *node, int *nb)
{
    if (node == table->sentinel) {
        *nb = 1;
        return RB_ERROR_OK;
    ...
    int left_err = nut_treetable_test(table, node->left, &nb_left);

    /* propagate the descendant errors all the way up */
    if (left_err != RB_ERROR_OK)
        return left_err;

    int right_err = nut_treetable_test(table, node->right, &nb_right);
...
{
    int x;
    int status = nut_treetable_test(table, table->root, &x);
    return status;
}
#endif
