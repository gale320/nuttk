...
static NutState get_node_at  (List *list, size_t index, Node **out);
static NutState add_all_to_empty    (List *l1, List *l2);
...
{
    conf->mem_alloc  = &nut_mem_malloc;//malloc;
    conf->mem_calloc = &nut_mem_calloc;//calloc;
    conf->mem_free   = &nut_mem_free;//free;
}
 ...
 *
 * @return NUT_OK if the creation was successful, or NUT_ERR_MALLOC if
 * the memory allocation for the new List failed.
 */
NutState nut_list_new(List **out)
{
    ListConf lc;
    nut_list_conf_init(&lc);
    return nut_list_new_conf(&lc, out);
}
 ...
 */
NutState nut_list_new_conf(ListConf const * const conf, List **out)
{
    //List *list = conf->mem_calloc(1, sizeof(List));
    List *list = nut_mem_calloc(1, sizeof(List));
...
{
    nut_list_remove_all_cb(list, cb);
    //list->mem_free(list);
    nut_mem_free(list);
}
...
{
    return nut_list_add_last(list, element);
}
...
NutState nut_list_add_first(List *list, void *element)
{
    ...
    Node *node = nut_mem_calloc(1, sizeof(Node));
    if (node == NULL)
        return NUT_ERR_MALLOC;
...
{
    Node *base;
    NutState stat = get_node_at(list, index, &base);

    if (stat != NUT_OK)
        return stat;

    //Node *new = list->mem_calloc(1, sizeof(Node));
    Node *new = nut_mem_calloc(1, sizeof(Node));
    if (!new)
        return NUT_ERR_MALLOC;
...
NutState nut_list_add_all(List *list1, List *list2)
{
    ...
    return nut_list_add_all_at(list1, list2, list1->size);
}
 ...
 * @param[in] list2 list from which the elements are being taken
 *
 * @return NUT_OK if the element were successfully added, or NUT_ERR_MALLOC if
 * the memory allocation for the new elements failed.
 */
static NutState add_all_to_empty(List *list1, List *list2)
{
    if (list2->size == 0)
        return NUT_OK;
 ...
 */
NutState nut_list_add_all_at(List *list1, List *list2, size_t index)
{
    if (list2->size == 0)
        return NUT_OK;
 ...
 *
 * @return NUT_OK if the elements were successfully moved, or NUT_ERR_OUT_OF_RANGE
 * if the index was not in range.
 */
NutState nut_list_splice(List *list1, List *list2)
{
    return nut_list_splice_at(list1, list2, list1->size);
}
 ...
 * if the index was not in range.
 */
NutState nut_list_splice_at(List *list1, List *list2, size_t index)
{
    if (list2->size == 0)
        return NUT_OK;
 ...
 *
 * @return NUT_OK if the element was successfully removed, or
 * NUT_ERR_OUT_OF_RANGE if the index was out of range.
 */
NutState nut_list_remove_at(List *list, size_t index, void **out)
{
    Node *node;
    NutState status = get_node_at(list, index, &node);
 ...
 *            freed
 *
 * @return NUT_OK if the elements were successfully removed and freed, or
 ...
 */
NutState nut_list_sublist(List *list, size_t b, size_t e, List **out)
{
    if (b > e || e >= list->size)
...
*/
    List *sub;
    NutState status = nut_list_new_conf(&conf, &sub);
    ...
    size_t i;
    for (i = b; i <= e; i++) {
        status = nut_list_add(sub, node->data);
        if (status != NUT_OK) {
            nut_list_destroy(sub);
            return status;
        }
        node = node->next;
    }
...
*/
    List *copy;
    NutState status = nut_list_new_conf(&conf, &copy);
    ...
    while (node) {
        status = nut_list_add(copy, node->data);
        if (status != NUT_OK) {
            nut_list_destroy(copy);
            return status;
        }
        node = node->next;
    }
 ...
 * memory allocation for the copy failed.
 */
NutState nut_list_copy_deep(List *list, void *(*cp) (void *e1), List **out)
{
    ListConf conf;
/*
    conf.mem_alloc  = list->mem_alloc;
...
*/
    ...
    while (node) {
        status = nut_list_add(copy, cp(node->data));
        if (status != NUT_OK) {
            nut_list_destroy(copy);
            return status;
        }
 ...
 *
 * @return NUT_OK if the array was successfully created, NUT_ERR_INVALID_RANGE if the
 * list is emtpy, or NUT_ERR_MALLOC if the memory allocation for the new array failed.
 */
NutState nut_list_to_array(List *list, void ***out)
{
   ...
   // void **array = list->mem_calloc(list->size, sizeof(void*));
    void **array = nut_mem_calloc(list->size, sizeof(void*));
    if (!array)
        return NUT_ERR_MALLOC;
...
{
    return nut_list_contains_value(list, element, nut_common_cmp_ptr);
}
 ...
 */
NutState nut_list_sort(List *list, int (*cmp) (void const *e1, void const *e2))
{
    void **elements;
    NutState status = nut_list_to_array(list, &elements);
 ...
 *                before the first
 */
void nut_list_sort_in_place(List *list, int (*cmp) (void const *e1, void const *e2))
{
 ...
 *               element
 */
void nut_list_foreach(List *list, void (*op) (void *e))
{
    Node *n = list->head;
 ...
 *                 be kept in the List
 *
 * @return NUT_OK if the list was filtered successfully, or NUT_ERR_OUT_OF_RANGE
 * if the List is empty.
 */
NutState nut_list_filter_mut(List *list, bool (*pred) (const void*))
{
    if (nut_list_size(list) == 0)
        return NUT_ERR_OUT_OF_RANGE;
 ...
 *
 * @return NUT_OK if the list was filtered successfully, NUT_ERR_OUT_OF_RANGE
 * if the list is empty, or NUT_ERR_MALLOC if the memory allocation for the
 * new list failed.
 */
NutState nut_list_filter(List *list, bool (*pred) (const void*), List **out)
{
    if (nut_list_size(list) == 0)
        return NUT_ERR_OUT_OF_RANGE;

    List *filtered = NULL;
    nut_list_new(&filtered);
    ...
    while (curr) {
        if (pred(curr->data)) {
            nut_list_add(filtered, curr->data);
        }
        curr = curr->next;
    }
...
/**
 * Removes the last returned element by <code>nut_list_iter_next()</code>
 ...
 * <code>nut_list_iter_next()</code> function without invalidating the
 * iterator.
 *
 * @note This function should only ever be called after a call to <code>
 * nut_list_iter_next()</code>.
...
NutState nut_list_iter_add(ListIter *iter, void *element)
{
	...
	Node *new_node = nut_mem_calloc(1, sizeof(Node));
    if (!new_node)
        return NUT_ERR_MALLOC;
...
/**
 * Replaces the last returned element by <code>nut_list_iter_next()</code>
...
/**
 * Returns the index of the last returned element by <code>nut_list_iter_next()
 * </code>.
 *
...
/**
 * Adds a new element to the list after the last returned element by
 * <code>nut_list_diter_next()</code> function (or before the element in
 * the list) without invalidating the  iterator.
 *
 * @note This function should only ever be called after a call to <code>
 * nut_list_diter_next()</code>.
 *
 * @param[in] iter the iterator on which this operation is being performed
 * @param[in] element the element being added to the list
 *
...
/**
 * Removes the last returned element by <code>nut_list_diter_next()</code>
 * function without invalidating the iterator and optionally sets the out
 * parameter to the value of the removed element.
 *
 * @note This function should only ever be called after a call to
 *       <code>nut_list_diter_next()</code>.
 *
 * @param[in] iter the iterator on which this operation is being performed
 * @param[out] out pointer to where the removed element is stored, or NULL
 *                 if it is to be ignored
...
/**
 * Replaces the last returned element by <code>nut_list_diter_next()</code>
 * with the specified element and optionally sets the out parameter to
 * the value of the replaced element.
 *
 * @note This function should only ever be called after a call to
...
/**
 * Returns the index of the last returned element by <code>nut_list_diter_next()
 * </code>.
 *
 * @param[in] iter the iterator on which this operation is being performed
 *
 ...
 * <code>nut_list_zip_iter_next()</code> and immediately before an element pair
 * that would be returned by a subsequent call to <code>nut_list_zip_iter_next()</code>
 * without invalidating the iterator.
 *
 * @note This function should only ever be called after a call to
 *       <code>nut_list_zip_iter_next()</code>
 *
...
/**
 * Removes and outputs the last returned element pair by <code>nut_list_zip_iter_next()
 ...
 *
 * @return NUT_OK if the element was removed successfully, or NUT_ERR_VALUE_NOT_FOUND.
 */
...
/**
 * Replaces the last returned element pair by <code>nut_list_zip_iter_next()</code>
 ...
 *
 * @return NUT_OK if the element was replaced successfully, or NUT_ERR_VALUE_NOT_FOUND.
 */
...
/**
 * Returns the index of the last returned element pair by <code>nut_list_zip_iter_next()</code>.
 *
 * @param[in] iter iterator on which this operation is being performed
 *
 ...
 *
 * @return NUT_OK if the node was found, or NUT_ERR_OUT_OF_RANGE if not.
 */
static NutState get_node_at(List *list, size_t index, Node **out)
{
...
static Node *get_node(List *list, void *element)
{
    Node *node = list->head;
    while (node) {
        if (nut_common_cmp_ptr(node->data, element) == 0)
            return node;
        node = node->next;
    }
    return NULL;
