...
NutState  resize          (HashTable *t, size_t new_capacity);
NutState  get_null_key    (HashTable *table, void **out);
NutState  add_null_key    (HashTable *table, void *val);
NutState  remove_null_key (HashTable *table, void **out);
...
{
    HashTableConf htc;
    nut_hashtable_conf_init(&htc);
    return nut_hashtable_new_conf(&htc, out);
}
...
void nut_hashtable_conf_init(HashTableConf *conf)
{
    ...
    conf->key_compare      = nut_common_cmp_str;
 ...
 *
 * @return NUT_OK if the mapping was successfully added, or NUT_ERR_ALLOC if the
...
NutState nut_hashtable_add(HashTable *table, void *key, void *val)
{
    ...
    if (table->size >= table->threshold) {
        if ((stat = resize(table, table->capacity << 1)) != NUT_OK)
            return stat;
    }
 ...
 *
 * @return NUT_OK if the key was found, or NUT_ERR_KEY_NOT_FOUND if not.
 */
 ...
 *
 * @return NUT_OK if a NULL key was found, or NUT_ERR_KEY_NOT_FOUND if not.
 */
NutState  get_null_key(HashTable *table, void **out)
{
 ...
 *
 * @return NUT_OK if the mapping was successfully removed, or NUT_ERR_KEY_NOT_FOUND
 ...
 *
 * @return NUT_OK if the resize was successful, or NUT_ERR_MAX_CAPACITY if maximum
 * capacity has been reached, or NUT_ERR_ALLOC if the memory allocation for the
 * new buffer failed.
 */
NutState  resize(HashTable *t, size_t new_capacity)
{
...
NutState nut_hashtable_get_values(HashTable *table, Array **out)
{
    ...
    Array *values;
    NutState stat = nut_array_new_conf(&ac, &values);
    if (stat != NUT_OK)
        return stat;
    ...
    for (i = 0; i <table->capacity; i++) {
        ...
        while (entry) {
            if ((stat = nut_array_add(values, entry->value)) == NUT_OK) {
                entry = entry->next;
            } else {
                nut_array_destroy(values);
                return stat;
            }
        }
    }
...
NutState nut_hashtable_get_keys(HashTable *table, Array **out)
{
    ...
    Array *keys;
    NutState stat = nut_array_new_conf(&vc, &keys);
    if (stat != NUT_OK)
        return stat;
    ...
    for (i = 0; i < table->capacity; i++) {
        ...
        while (entry) {
            if ((stat = nut_array_add(keys, entry->key)) == NUT_OK) {
                entry = entry->next;
            } else {
                nut_array_destroy(keys);
                return stat;
            }
        }
    }
 ...
 */
void nut_hashtable_foreach_key(HashTable *table, void (*fn) (const void *key))
{
    size_t i;
    for (i = 0; i <table->capacity; i++) {
        TableEntry *entry = table->buckets[i];
 ...
 *               table
 */
void nut_hashtable_foreach_value(HashTable *table, void (*fn) (void *val))
{
    size_t i;
    for (i = 0; i <table->capacity; i++) {
        TableEntry *entry = table->buckets[i];
 ...
 */
NutState nut_hashtable_iter_next(HashTableIter *iter, TableEntry **te)
{
    if (!iter->next_entry)
        return NUT_ITER_END;
...
/**
 * Removes the last returned entry by <code>nut_hashtable_iter_next()</code>
...
{
    return nut_hashtable_remove(iter->table, iter->prev_entry->key, out);
}
...
uint64_t nut_hashtable_hash(const void *key, int len, uint32_t seed)
{
 ...
 */
uint64_t nut_hashtable_hash_ptr(const void *key, int len, uint32_t seed)
{
